...
| 作成日 | 2026-09-11 |
| バージョン | v3.1 |
| 最終更新日 | 2026-09-20（v3.1：㉟新設Step 32「QRコード表示機能」（`package.json`・`MyProfileEditor.tsx`更新、`qrcode.react` 採用、SDD ADR-033 準拠）を旧Step 32（デプロイ設定）の直前に挿入。旧Step 32 を新Step 33 に繰り下げ。v3.0 までの変更履歴はそのまま引き継ぐ。) |

| ステップ数 | 33（Step 1〜33。旧 Step 22 の欠番は既に解消済み） |
...
**対象ファイル**
`package.json`（更新） / `client/src/components/student/MyProfileEditor.tsx`（更新）

**前提条件**
Step 27（`MyProfileEditor.tsx`）完了

**SDD 参照**
§3.1 技術スタック（`qrcode.react`） / §9.11 MyProfileEditor フォーム項目 / Appendix E: ADR-033

---

`package.json`（更新）:
- dependencies に `qrcode.react`（4.x）を追加する

`MyProfileEditor.tsx`（更新）:
- `import { QRCodeSVG } from 'qrcode.react'` を追加する
- `externalPublic` が true の場合に表示している固定URL（テキスト表示＋コピーボタン）の下に、`<QRCodeSVG value={publicUrl} size={160} />` を追加する
- QRコードの表示・非表示は既存の `externalPublic` の状態にそのまま連動させる（新しい state は追加しない）

---

### Step 33 — デプロイ設定ファイル（学生機能反映済み）
