...
| 作成日 | 2026-09-11 |
| バージョン | v2.5 |
| 最終更新日 | 2026-09-20（v2.5：㉙Step 11・Step 31 の `npx shadcn@latest add` コマンドに `switch`・`alert-dialog` が含まれていなかったため追加。両コンポーネントは Step 27 の `MyProfileEditor.tsx`・`MyAchievements.tsx` で使用しており、これらが漏れていると実際にビルドエラーになることをローカル動作確認で確認済み。v2.4 までの変更履歴はそのまま引き継ぐ。) |

| ステップ数 | 31（Step 1〜31。旧 Step 22 の欠番は今回の振り直しで解消済み） |
...
**shadcn/ui の初期化とコンポーネントの追加**（CLI 実行、`npm install` 後に実施）:
```bash
# 初期化（components.json が既存の場合は上書き不要 → N を選択）
npx shadcn@latest init

# コンポーネント追加
npx shadcn@latest add button card dialog badge input label select textarea checkbox alert scroll-area switch alert-dialog
```
→ `client/src/components/ui/` に各コンポーネントが生成される

> **⚠️ 注意**：`shadcn@latest init` は `components.json` が既存の場合に上書き確認が出る。既存の設定（`client/src/globals.css`・`tailwind.config.ts` の参照）を維持するため `N`（上書きしない）を選択する。
...
`Navbar`・`CategoryCard`・`CategoryModal`・`StickerModal`・`LoginScreen`・`AdminPanel` 等の子コンポーネントは Step 13〜19 でこれから作られるため、このステップの時点ではまだ存在しない。
...
return (
  <div className="p-4 text-sm text-muted-foreground">
    読み込み中... （UI は Step 19 で統合されます）
  </div>
)
```

state・`authFetch`・データフェッチ・検索フィルタリングのロジック自体はここで完成させてよい（`filteredCategories`・`filteredStickers` の算出まで含む）。ただし、それらを画面に描画する JSX は書かない。**代替の簡易 UI をインラインで作らないこと**（Step 19 で二度手間になり、`App.tsx` への書き込みが増える原因になるため）。

`App.tsx` への書き込みは、本ステップと Step 19・Step 29・Step 30 の合計 4 回に固定する。子コンポーネントが 1 つ完成するたびに `App.tsx` を書き換えることはしない。
...
`client/src/types.ts` は Step 12 で Appendix B の型（`Category`・`Sticker`・`Course`・`StickerCategory`・`User`・`CurrentUser`・`UserRole`・`StickerType`・`StickerLevel`）を含めて作成済みのため、本ステップでの追加作業はない。
...
- `SkillChip.tsx`（本ステップ）・`StickerModal.tsx`（Step 15）の両方がこのファイルから import する。定義を複製しない。
...
種別バッジの定義（`STICKER_TYPE_BADGE_CLASS`・`STICKER_TYPE_LABEL`）は Step 14 で作成した `client/src/lib/stickerTypeLabels.ts` を import して使う。`StickerModal.tsx` 内で重複定義しない。
...
> **⚠️ 実装上の注意**：Step 17 では CategoryEditor / StickerEditor / UserEditor の import とコメントアウト解除を**Step 18・19 完了後に行う**。Step 17 単体の実装時点ではコメントアウトしたままにしておくこと。Step 18・19 が完了した段階で AdminPanel の該当箇所のコメントを外す。コメントアウトを解除する際、`CategoryEditor`・`StickerEditor`へは本ステップで `AdminPanel` が受け取っている `authFetch` をそのまま props として渡す（`<CategoryEditor ... authFetch={authFetch} />` のように。`UserEditor` は画像アップロードを行わないため `authFetch` を渡さない）。
...
**`CategoryEditor.tsx`**  
props: `{ cat, onSave, onDelete, onClose, authFetch }`（`authFetch` の型は Step 17 と同様 `import type { authFetch as authFetchType } from '../../App'` の上で `authFetch: typeof authFetchType` とする。画像アップロード（`POST /api/upload?prefix=categories`）に使用する）（cat が null の場合は新規作成モード）
...
### Step 19 — StickerEditor と CourseRow、および App.tsx 統合（v9.0コンポーネント編）

**対象ファイル**  
`client/src/components/admin/StickerEditor.tsx` / `client/src/components/admin/CourseRow.tsx` / `client/src/App.tsx`（更新）

**前提条件**  
Step 12〜18 完了（Navbar・CategoryCard・SkillChip・CategoryModal・StickerModal・LoginScreen・AdminPanel・CategoryEditor・UserEditor が全て存在すること）
...
**`StickerEditor.tsx`**  
props: `{ sticker, categories, onSave, onDelete, onClose, canEdit, authFetch }`（`authFetch` の型は Step 17 と同様 `import type { authFetch as authFetchType } from '../../App'` の上で `authFetch: typeof authFetchType` とする。画像アップロード（`POST /api/upload?prefix=stickers`）に使用する）
...
Step 12 で骨格のみ実装した `App.tsx` に、Step 13〜18 で作成した子コンポーネントを組み込み、プレースホルダーの JSX を置き換える。state・`authFetch`・データフェッチ・検索フィルタリングのロジックは Step 12 のものをそのまま流用し、変更しない。
...
このステップ以降、Step 20〜28 は `App.tsx` に触れない。次に `App.tsx` を更新するのは Step 29 である。
...
## 学生機能拡張（Step 21〜30、v10.0）

### Step 21 — 学生関連ライブラリ（CSVインポート・学生用ミドルウェア）
...
> **`requireKioskToken` は実装しない**。SkillsStampConnect v11.0 ではキオスクトークン方式を採用せず、外部公開は認証不要の固定URL（`GET /api/public/profile/:loginId`、Step 25）で実現する（→ SDD Appendix E: ADR-032）。

---

### Step 22 — 学生管理 API（CSVインポート・一覧・状態変更）
...
全エンドポイントに `authenticate + requireAdmin` を適用する（実績記録関連は Step 24 で別ファイルに実装し、🔑 職員全員に開放する）。
...
### Step 23 — マイページ API（プロフィール・実績・公開設定）
...
### Step 24 — 実績記録 API・スタンプ保有者 API・公開学生一覧 API
...
### Step 25 — 外部公開プロフィール API（v11.0。旧「キオスクトークン API・キオスク公開 API」を置き換え）

**対象ファイル**
`routes/publicProfile.js`（新規）

**前提条件**
Step 2・4・21・23・24 完了
...
> **admin 向けのトークン発行・失効エンドポイントは実装しない**。`kiosk_tokens` テーブル・`POST/DELETE /api/kiosk-tokens`・`GET /api/kiosk/*`・`requireKioskToken` はいずれも SkillsStampConnect には存在しない（v10.0 の旧設計であり、Step 2・4・21 でも作成していない）。就職説明会・オンラインイベントいずれも、本エンドポイントの URL（`/api/public/profile/:loginId`）をQRコード化して配布するだけで閲覧できる（→ SDD Appendix E: ADR-032）。

---

### Step 26 — Express サーバー本体の更新
...
Step 12 で定義した `UserRole`（`'admin' | 'user'`）に `'student'` を追加し、`'admin' | 'user' | 'student'` とする。あわせて `CurrentUser` に `mustChangePassword: boolean` を追加する。

さらに、`MyProfileEditor.tsx`・`MyAchievements.tsx` の props 型付けに使う以下の2つの型を新規に定義する（`routes/me.js`（Step 23）のレスポンス形に対応させる）:
...
**`MyProfileEditor.tsx`**
props: `{ profile, onSave, authFetch }`（`authFetch` の型は Step 17 と同様 `import type { authFetch as authFetchType } from '../../App'` の上で `authFetch: typeof authFetchType` とする。画像アップロード（`POST /api/upload?prefix=students`）に使用する）
...
- 本コンポーネントは Step 30 の外部公開プロフィール画面（`GET /api/public/profile/:loginId`、認証不要）でもレイアウトをそのまま再利用する。`fetchFn` を差し替えられる設計にしておくことで、認証あり／なしの両方の呼び出し元に対応する

**`Navbar.tsx`（更新）**：ギャラリー／学生一覧／管理画面の切り替えに「学生一覧」を追加し、クリックで `StudentDirectory` ビューに切り替える。あわせて `currentUser.role === 'student'` の場合のみ「マイページ」導線を追加し、クリックで `myPage` ビューに切り替える。Step 27 で `UserRole` に `'student'` が追加されたことを受け、`ROLE_BADGE_CLASS`・`ROLE_LABEL`（Step 13 で `'admin' | 'user'` の2キーのみ定義済み）に `student` キーを追加する。

---

### Step 29 — フロントエンド 管理画面拡張、および App.tsx 統合（学生系コンポーネント編）
...
**前提条件**
Step 19（App.tsx v9.0統合済み）・22・24・25・27・28 完了（PasswordChangeScreen・MyProfileEditor・MyAchievements・StudentDirectory・StudentProfilePage が全て存在すること）
...
Step 19 で v9.0コンポーネントを統合した `App.tsx` に対し、以下のみを追加する。Step 19 時点の配線（Navbar・CategoryCard・CategoryModal・StickerModal・LoginScreen・AdminPanel）は変更しない。
...
- `StickerModal` に「取得学生一覧」セクションが追加されたことを受け、`GET /api/stickers/:id/holders` の呼び出しに必要な `authFetch` を渡す（`StickerModal` 側の実装は Step 27〜28 で完了済み）
- 表示名クリックで `StudentProfilePage` に遷移する導線のため、`view` に `'studentProfile'` と対象 `loginId` を保持する state を追加する

このステップ以降、`App.tsx` を更新するのは Step 30（外部公開プロフィール専用ルート）のみである。

---

### Step 30 — 外部公開プロフィール専用ルート（v11.0。旧「キオスクモード専用ルート」を置き換え）

**対象ファイル**
`client/src/App.tsx`（更新）

**前提条件**
Step 25・29（App.tsx 学生系統合済み）完了
...
2. **セットアップ手順**（§11.3 の手順を bash コマンドで記述）:
   ...
   npm install
   npx shadcn@latest init  # N を選択（上書き不要）
   npx shadcn@latest add button card dialog badge input label select textarea checkbox alert scroll-area switch alert-dialog
...
上記の運用ドキュメントには、Step 1〜30 で実装した学生機能（学生アカウント・実績記録・QRコード外部公開）も以下の内容で含める。
