...
| 作成日 | 2026-09-11 |
| バージョン | v2.4 |
| 最終更新日 | 2026-09-20（v2.4：㉘Step 30 の App.tsx 更新に、外部公開ビュー判定の早期 `return` を全 `useState`・`useEffect`・`useCallback` 呼び出しの後に置くべき旨（React のフックのルール違反を防ぐため）を明記。実装時に、フック呼び出しより前に早期 `return` を置いてしまい違反する実装を一度作ってしまったための追記。v2.3 までの変更履歴はそのまま引き継ぐ。) |
